...
const verifyCompRAM = (ram, mth) =>
  ram.type === mth.ddr && ram.velocity >= mth.ramMinVelocity
...
function buildInfoAboutComponent(item, title, components, componentKey) {
  ...
  btn.addEventListener('click', () => {
    if (
      setup[componentKey].hasOwnProperty('wats') &&
      componentKey !== 'cooling'
    )
      setup.totalWats -= setup[componentKey].wats
...
function handleSelection(item, title) {
  const updateTotalPrice = price => {
    ...
    total.innerHTML = `Total: R$ ${priceParser(
      Math.round(setup.totalPrice * 100) / 100,
    )}`
  }
...
function buildModal(title, items) {
                    ...
                    <div class="component">
                        <div class="component-all">
                            ...
                            <div class="component-info">
                                ...
                                <span class="component-price">R$ ${priceParser(
                                  item.price,
                                )}</span>
                            </div>
                        </div>
  ...
  function isCompatible(item, title) {
    if (title === 'CPU' && setup.motherboard)
      return verifyCompCPU(item, setup.motherboard)
    if (title === 'Placa de Vídeo' && setup.motherboard)
      return verifyCompGPU(item, setup.motherboard)
    if (title === 'RAM' && setup.motherboard)
      return verifyCompRAM(item, setup.motherboard)
    if (title === 'HD/SSD' && setup.motherboard)
      return verifyCompSTR(item, setup.motherboard)
    if (title === 'Placa Mãe') {
      return (
        (!setup.cpu || verifyCompCPU(setup.cpu, item)) &&
...
function updateSetup(cpus, gpus, rams, storages, motherboards, coolings) {
  if (setup.cpu)
    updateComponentDisplay('.cpu-tr', setup.cpu, 'CPU', cpus, 'cpu')
  if (setup.gpu)
    updateComponentDisplay('.gpu-tr', setup.gpu, 'Placa de Vídeo', gpus, 'gpu')
  if (setup.ram)
    updateComponentDisplay('.ram-tr', setup.ram, 'RAM', rams, 'ram')
  if (setup.storage.length > 0)
    updateComponentDisplay(
      '.storage-tr',
      setup.storage,
      'HD/SSD',
